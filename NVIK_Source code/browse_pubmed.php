...
if ($fname == 'Pubmed_id') {
    side_bar();
    $all_rec = file("nipah_pubmed_sort_updated.txt");
    $rec = count($all_rec);
    print "<b><center>Pubmed_id (Unique) = 38</center></b><br>";
    ...
    print "<thead><tr>
        ...
        <th align='center' bgcolor='#3E6990'><font color='white' size='3'><b>Pubmed_id</b></font></th>
        <th align='center' bgcolor='#3E6990'><font color='white' size='3'><b>No. of Entries</b></font></th>
        </tr></thead>";
    
  $result = fopen('nipah_pubmed_sort_updated.txt', 'r'); 
$sr_no = 1;
...
while (!feof($result)) {
    ...
    if (!empty($name)) {
        ...
        $query = "SELECT NVIC_ID, COUNT(*) AS entry_count 
                  FROM data_2024 
                  WHERE Pubmed_id = '$name'";
        $res = mysql_query($query) or die("Query Failed: " . mysql_error());

        // Fetch data
        if ($row = mysql_fetch_assoc($res)) {
            $cls_id = $row['NVIC_ID']; // Fetch NVIC_ID value
            $entry_count = $row['entry_count']; // Fetch count of Pubmed_id
            ...
            echo "<td><a href='search-nidb.php?Pubmed_id=" . urlencode($name) . "&type=Pubmed_id'><b><i>$name</i></b></a></td>"; // Pubmed_id
            echo "<td>$entry_count</td>"; // No. of Entries column
            echo "</tr>";
        } else {
            // Row for Pubmed_id  not found in the database
            echo "<tr>";
            echo "<td>$sr_no</td>"; // Serial number column
            echo "<td>Not Found</td>"; // NVIC_ID column
            echo "<td><i>$name</i></td>"; // Pubmed_id  Column
