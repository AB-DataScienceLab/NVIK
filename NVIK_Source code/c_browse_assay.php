...
if ($fname == 'Assay_type') {
    side_bar();
    $all_rec = file("nipah_assay_type_sort_updated.txt");
    $rec = count($all_rec);
    print "<b><center>Assay_Type (Unique) = 16</center></b><br>";
    ...
    print "<thead><tr>
        ...
        <th align='center' bgcolor='#3E6990'><font color='white' size='3'><b>Assay_type</b></font></th>
        <th align='center' bgcolor='#3E6990'><font color='white' size='3'><b>No. of Entries</b></font></th>
        </tr></thead>";
    
  $result = fopen('nipah_assay_type_sort_updated.txt', 'r'); 
$sr_no = 1;
...
while (!feof($result)) {
    ...
    if (!empty($name)) {
        ...
        if ($row = mysql_fetch_assoc($res)) {
            ...
            echo "<td><a href='search-nidb.php?Assay_type=" . urlencode($name) . "&type=Assay_type'><b><i>$name</i></b></a></td>"; // Assay_type 
            echo "<td>$entry_count</td>"; // No. of Entries column
            echo "</tr>";
        } else {
            // Row for Assay_type  not found in the database
            echo "<tr>";
            echo "<td>$sr_no</td>"; // Serial number column
            echo "<td>Not Found</td>"; // NVIC_ID column
            echo "<td><i>$name</i></td>"; // Assay_type  Column
